...
type UseForumCommandsParams = {
  ...
  subTopics: SubTopic[];
  posts: Post[];
  setTopicDirectoryIndex: Dispatch<SetStateAction<TopicDirectorySnapshot | null>>;
...
const isAdminRole = (role: User["role"]) => role === "Admin" || role === "SuperAdmin";
...
export const useForumCommands = ({
  ...
  const syncTopicDirectoryIndex = useCallback(
    async (nextTopics: Topic[], nextSubTopics: SubTopic[]) => {
      try {
        const snapshot = await forumSearchIndexService.publishTopicDirectoryIndex(
          nextTopics,
          nextSubTopics,
  ...
  const createSubTopic = useCallback(
    async (input: {
      ...
      if (!canCreateSubTopicForTopic(parentTopic, currentUser, authenticatedAddress)) {
        return {
          ok: false,
          error: "You do not have permission to create a sub-topic under this main topic.",
        };
      }
  ...
  const updateTopicSettings = useCallback(
    async (input: {
      ...
      try {
        ...
        setTopics((current) =>
          current.map((topic) => (topic.id === target.id ? updatedTopic : topic))
        );
        void syncTopicDirectoryIndex(nextTopics, subTopics);
        return { ok: true };
      } catch (error) {
        return {
          ok: false,
          error: error instanceof Error ? error.message : "Failed to update main topic.",
        };
      }
    },
    [
  ...
  const updateSubTopicSettings = useCallback(
    async (input: {
      ...
      if (!isModeratorRole(currentUser.role)) {
        return {
          ok: false,
          error: "Only moderators, admins and the super admin can manage sub-topics.",
        };
      }
      ...
      try {
        await forumQdnService.publishSubTopic(updatedSubTopic, currentUser.username);
        ...
        return {
          ok: false,
          error: error instanceof Error ? error.message : "Failed to update sub-topic.",
        };
      }
    },
    [
  ...
  const upsertRoleAssignment = useCallback(
    async (input: {
      ...
      if (
        ...
        return { ok: false, error: "Only the super admin can manage forum roles." };
      }

      if (address === roleRegistry.superAdminAddress) {
        return { ok: false, error: "The super admin address is fixed and cannot be reassigned." };
      }
      ...
      try {
        ...
        return {
          ok: false,
          error:
            error instanceof Error ? error.message : "Failed to update forum role registry.",
        };
      }
    },
    [
  ...
  const removeRoleAssignment = useCallback(
    async (address: string): Promise<ForumMutationResult> => {
      ...
      const nextRegistry: ForumRoleRegistry = {
        ...roleRegistry,
        admins: roleRegistry.admins.filter((entry) => entry !== normalizedAddress),
  ...
  const createPost = useCallback(
    async (input: {
      ...
      if (targetSubTopic.visibility === "hidden" && !isModeratorRole(currentUser.role)) {
      ...
      try {
        ...
        setUsers((current) => ensureCurrentUserPresent(current, currentUser));
        void syncThreadSearchIndex(input.subTopicId, nextPosts);
        void syncTopicDirectoryIndex(topics, nextSubTopics);
        return { ok: true };
      } catch (error) {
        return {
  ...
  const updatePost = useCallback(
    async (input: {
      ...
      try {
        ...
        void syncThreadSearchIndex(updatedPost.subTopicId, nextPosts);
        return { ok: true };
      } catch (error) {
        return {
          ok: false,
  ...
  const deletePost = useCallback(
    async (postId: string): Promise<ForumMutationResult> => {
      ...
      if (target.authorUserId !== currentUser.id && !isModeratorRole(currentUser.role)) {
        return { ok: false, error: "Only owner or moderator can delete this post." };
      }
      ...
      try {
        ...
        void syncThreadSearchIndex(target.subTopicId, nextPosts);
        return { ok: true };
      } catch (error) {
        return {
          ok: false,
