...
export const buildForumStructureSearchIndex = (
  ...
  return {
    ...
    subTopicEntries: subTopics.map((subTopic) => {
      ...
      const authorName = userMap.get(subTopic.authorUserId) ?? subTopic.authorUserId;
  ...
  const filteredTopics = topics
    .map((topic) => {
      ...
      const nextSubTopics = topicMatched
        ? topic.subTopics
        : topic.subTopics.filter((subTopic) => subTopicMatches.has(subTopic.id));
    ...
    .filter((topic) => topicMatches.has(topic.id) || topic.subTopics.length > 0);
...
export const buildThreadPostSearchIndex = (
  ...
  return {
    entries: posts.map((post) => ({
      postId: post.id,
      haystack: normalizeText(
        [post.content, userMap.get(post.authorUserId) ?? post.authorUserId].join(" ")
      ),
    })),
  };
};
