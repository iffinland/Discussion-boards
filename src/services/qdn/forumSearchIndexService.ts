...
const parseTopicDirectoryPayload = (raw: unknown): TopicDirectoryPayload | null => {
  if (!isObject(raw) || raw.type !== "topic-directory-index" || !isObject(raw.snapshot)) {
    return null;
  }

  const topics = Array.isArray(raw.snapshot.topics) ? raw.snapshot.topics : [];
  const subTopics = Array.isArray(raw.snapshot.subTopics) ? raw.snapshot.subTopics : [];
    ...
    snapshot: {
      ...
      topics: topics
        ...
        .map((item) => ({
          ...
          status: (item.status === "locked" ? "locked" : "open") as Topic["status"],
          visibility: (
            item.visibility === "hidden" ? "hidden" : "visible"
          ) as Topic["visibility"],
          subTopicAccess: (
            item.subTopicAccess === "moderators" ||
            item.subTopicAccess === "admins" ||
            item.subTopicAccess === "custom"
              ? item.subTopicAccess
              : "everyone"
          ) as Topic["subTopicAccess"],
          allowedAddresses: Array.isArray(item.allowedAddresses)
            ? item.allowedAddresses.filter(
                (address): address is string => typeof address === "string" && Boolean(address.trim())
              )
            : [],
        }))
      ...
      subTopics: subTopics
        .filter((item) => isObject(item))
        .map((item) => ({
          subTopicId: typeof item.subTopicId === "string" ? item.subTopicId : "",
          topicId: typeof item.topicId === "string" ? item.topicId : "",
          title: typeof item.title === "string" ? item.title : "",
          description: typeof item.description === "string" ? item.description : "",
          status: (
          ...
          visibility: (
            item.visibility === "hidden" ? "hidden" : "visible"
          ) as SubTopic["visibility"],
          authorUserId:
            typeof item.authorUserId === "string" ? item.authorUserId : "",
          lastPostAt: typeof item.lastPostAt === "string" ? item.lastPostAt : "",
        }))
...
const parseThreadIndexPayload = (raw: unknown): ThreadIndexPayload | null => {
  if (!isObject(raw) || raw.type !== "thread-search-index" || !isObject(raw.snapshot)) {
    return null;
  }

  const posts = Array.isArray(raw.snapshot.posts) ? raw.snapshot.posts : [];

  if (typeof raw.snapshot.subTopicId !== "string" || !raw.snapshot.subTopicId.trim()) {
    return null;
  }
...
export const forumSearchIndexService = {
  ...
  async publishThreadIndex(subTopicId: string, posts: Post[], ownerName?: string) {
    const resolvedOwner = await resolveOwnerName(ownerName);
    const updatedAt = Date.now();
    const identifier = `${THREAD_INDEX_PREFIX}${subTopicId}`;
